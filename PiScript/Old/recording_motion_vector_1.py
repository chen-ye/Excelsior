...
with picamera.PiCamera() as camera:
    ...
    filename = 'array_' + time_at_start_HMS
    with picamera.array.PiMotionArray(camera) as stream:
        camera.resolution = (640, 480)
        camera.framerate = 60 #max is 30
        camera.start_recording('/dev/null', format='h264', motion_output=stream)
        camera.wait_recording(1) #how many seconds worth of recording
        camera.stop_recording()
	
	np.save(filename, stream.array) 

print np.load('array.npy')
#print "Array saved as: %s" % filename 
#print 'array_'+filename+'.npy'.shape
